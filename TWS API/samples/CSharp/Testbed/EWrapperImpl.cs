    ...
    //! [ewrapperimpl]
        ...
        public virtual void openOrder(int orderId, Contract contract, Order order, OrderState orderState)
        {
            Console.WriteLine("OpenOrder. PermID: " + Util.LongMaxString(order.PermId) + ", ClientId: " + Util.IntMaxString(order.ClientId) + ", OrderId: " + Util.IntMaxString(orderId) + 
                ", Account: " + order.Account + ", Symbol: " + contract.Symbol + ", SecType: " + contract.SecType + ", Strike: " + Util.DoubleMaxString(contract.Strike) + " , Exchange: " + contract.Exchange + ", Action: " + order.Action + 
            ...
            printSoftDollarTier(order.Tier);
        ...
        public void printTagValueList(List<TagValue> tagValueList, string listName) 
        {
        ...
        public void printConditions(List<OrderCondition> conditions) 
        {
            if (conditions != null && conditions.Count > 0)
            {
                Console.Write("\tConditions:{");
                foreach (OrderCondition orderCondition in conditions)
                {
                    Console.Write(orderCondition.ToString() + "; ");
                }
                Console.WriteLine("}");
            }
        }
        ...
        public void printDeltaNeutralContract(DeltaNeutralContract deltaNeutralContract) 
        {
            Console.WriteLine("\tDeltaNeutralContract: {conid=" + Util.IntMaxString(deltaNeutralContract.ConId) + ", delta=" + Util.DoubleMaxString(deltaNeutralContract.Delta) + 
                ", price=" + Util.DoubleMaxString(deltaNeutralContract.Price) + "}");
        }

        public void printComboLegs(List<ComboLeg> comboLegList)
        {
            if (comboLegList != null && comboLegList.Count > 0)
            {
                Console.Write("\tComboLegs:{");
                foreach (ComboLeg comboLeg in comboLegList)
                {
                Console.Write("conId: " + Util.IntMaxString(comboLeg.ConId) + ", action: " + comboLeg.Action + ", ratio: " + Util.IntMaxString(comboLeg.Ratio) + 
        ...
        public void printOrderComboLegs(List<OrderComboLeg> orderComboLegList) 
        {
            if (orderComboLegList != null && orderComboLegList.Count > 0)
            {
                Console.Write("\tOrderComboLegs:{");
                foreach (OrderComboLeg orderComboLeg in orderComboLegList)
                {
                    Console.Write("price: " + Util.DoubleMaxString(orderComboLeg.Price) + "; ");
                }
                Console.WriteLine("}");
            }
        }

        public void printOrderAllocationsList(List<OrderAllocation> orderAllocationList)
        {
            if (orderAllocationList != null && orderAllocationList.Count > 0)
            {
                Console.Write("\tOrderAllocationList: {");
                foreach (OrderAllocation orderAllocation in orderAllocationList)
                {
        ...
        public void printContractMsg(Contract contract)
        {
            ...
            Console.WriteLine("\tLastTradeDate: " + contract.LastTradeDate);
        ...
        public void printContractDetailsIneligibilityReasonList(List<IneligibilityReason> ineligibilityReasonList)
        {
            if (ineligibilityReasonList != null && ineligibilityReasonList.Count > 0)
            {
                Console.Write("\tIneligibilityReasonList: {");
                foreach (IneligibilityReason ineligibilityReason in ineligibilityReasonList)
                {
        ...
        {
            Console.WriteLine("CompletedOrder. PermID: " + Util.LongMaxString(order.PermId) + ", ParentPermId: " + Util.LongMaxString(order.ParentPermId) + ", Account: " + order.Account + ", Symbol: " + contract.Symbol + ", SecType: " + contract.SecType + 
