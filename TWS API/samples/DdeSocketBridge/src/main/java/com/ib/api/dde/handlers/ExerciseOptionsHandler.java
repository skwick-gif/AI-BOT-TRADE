...
public class ExerciseOptionsHandler extends BaseHandler {
    ...
    public byte[] handleExerciseOptionsRequest(String requestStr, byte[] data) {
        ...
        clientSocket().exerciseOptions(request.requestId(), request.contract(), request.exerciseAction(), 
                request.exerciseQuantity(), request.account(), request.override(), request.manualOrderTime(), 
                request.customerAccount(), request.professionalCustomer()); 
        return null;
    }
    ...
    private class ExerciseOptionsRequestParser extends RequestParser {
        ...
        public ExerciseOptionsRequest parseExerciseOptionsRequest(String requestStr, byte[] data) {
            ...
            boolean professionalCustomer = false;
            ...
            if (messageTokens.length > 1) {
                ...
                if (messageParams.length > 4) {
                	manualOrderTime = messageParams[4];
                }
                if (messageParams.length > 5) {
                    customerAccount = messageParams[5];
                }
                if (messageParams.length > 6) {
                    professionalCustomer = getBooleanFromString(messageParams[6]);
                }
            }
            return new ExerciseOptionsRequest(requestId, contract, account, exerciseAction, exerciseQuantity, override, manualOrderTime, customerAccount, professionalCustomer, requestStr);
        }
    }
}
