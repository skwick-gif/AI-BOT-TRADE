...
public class Utils {
    ...
    private static <T> ArrayList<String> createTableItem(T data, boolean bValue1, boolean bValue2, DdeRequestType ddeRequestType) {
        ...
        if (data instanceof String) {
            return MiscUtils.createTableItem((String)data);
        }
        return null;
    }
    ...
    public static String conditionsToString(List<OrderCondition> orderConditions) {
        ...
        if (orderConditions != null) {
            int size = orderConditions.size();
            for (int i = 0; i < size; i++) {
                OrderCondition orderCondition = orderConditions.get(i);
                sb.append(orderCondition.toString());
                if (i < (size - 1)) {
                    sb.append(orderCondition.conjunctionConnection() ? " and " : " or ");
                }
            }
        }
        return sb.toString();
    ...
    public static String ineligibiltyReasonListToString(List<IneligibilityReason> ineligibilityReasonList) {
        StringBuilder sb = new StringBuilder();
        if (ineligibilityReasonList != null) {
            for (IneligibilityReason ineligibilityReason : ineligibilityReasonList) {
                sb.append("[id: ").append(ineligibilityReason.id()).append(", description: ").append(ineligibilityReason.description()).append("];");
            }
        }
        return sb.toString();
    }
