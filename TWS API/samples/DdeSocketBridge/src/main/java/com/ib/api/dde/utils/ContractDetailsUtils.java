...
public class ContractDetailsUtils {
    ...
    static ArrayList<String> createTableItem(ContractDetails contractDetails) {
        ...
        item.add(Utils.toString(contractDetails.fundFrontLoad()));
        item.add(Utils.toString(contractDetails.fundBackLoad()));
        item.add(Utils.toString(contractDetails.fundBackLoadTimeInterval()));
        item.add(Utils.toString(contractDetails.fundManagementFee()));
        item.add(Utils.toString(contractDetails.fundClosed()));
        item.add(Utils.toString(contractDetails.fundClosedForNewInvestors()));
        item.add(Utils.toString(contractDetails.fundClosedForNewMoney()));
        item.add(Utils.toString(contractDetails.fundNotifyAmount()));
        item.add(Utils.toString(contractDetails.fundMinimumInitialPurchase()));
        item.add(Utils.toString(contractDetails.fundSubsequentMinimumPurchase()));
        item.add(Utils.toString(contractDetails.fundBlueSkyStates()));
        item.add(Utils.toString(contractDetails.fundBlueSkyTerritories()));
        item.add(Utils.toString(contractDetails.fundDistributionPolicyIndicator() != null ? contractDetails.fundDistributionPolicyIndicator().getName() : ""));
        item.add(Utils.toString(contractDetails.fundAssetType() != null ? contractDetails.fundAssetType().getName() : ""));
        String ineligibilityReasons = Utils.ineligibiltyReasonListToString(contractDetails.ineligibilityReasonList());
        if (Utils.isNotNull(ineligibilityReasons)) {
            item.add(Utils.LONGVALUE + RequestParser.PARAM_SEPARATOR + Utils.toString(Utils.calcNumOfChunks(ineligibilityReasons, 255)));
            item.addAll(Utils.chunkStringByLength(ineligibilityReasons, 255)); // long string
        }
