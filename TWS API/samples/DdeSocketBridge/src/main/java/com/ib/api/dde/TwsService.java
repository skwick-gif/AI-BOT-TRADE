...
public class TwsService {
    ...
    private final String m_connectOptions;
    ...
    public TwsService(String host, int port, int clientId, String connectOptions,
    ...
    public <T> T sendDdeToTws(String topic, String requestStr, T data, boolean withData) {
        ...
        switch(requestType) {
            // get next valid id
            case GET_NEXT_VALID_ID:
                System.out.println("Getting next valid id: " + m_nextValidId);
                return (T) Utils.dataListToByteArray(Arrays.asList(String.valueOf(m_nextValidId)));
            ...
            case REQUEST_HISTORICAL_TICKS:
              return data == null ? returnNotNullOrError(requestType.topic(), (T) m_historicalTicksHandler.handleHistoricalTicksArrayRequest(requestStr)) : (T) m_historicalTicksHandler.handleHistoricalTicksRequest(requestStr, (byte[]) data);
    ...
    public boolean connect() throws InterruptedException {
        System.out.println("Connecting to host [" + m_host + "] port [" + m_port + "] clientId [" + m_clientId + "] connectOptions [" + m_connectOptions + "]");

        m_clientSocket.setConnectOptions(m_connectOptions);
     ...
     *                                          Next Valid Id
    /* *****************************************************************************************************/
    /** Method updates next valid id */
    public void updateNextValidId(int orderId) {
        // initial value
