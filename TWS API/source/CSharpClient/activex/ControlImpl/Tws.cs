...
namespace TWSLib
{
    ...
    public class Tws : EWrapper, ITws, IDisposable
    {
        ...
        void ITws.cancelOrder(int id, IOrderCancel orderCancel)
        {
            socket.cancelOrder(id, (OrderCancel)(orderCancel as ComOrderCancel));
        }
        ...
        void ITws.connect(string host, int port, int clientId, string connectOptions, bool extraAuth)
        {
            this.socket.SetConnectOptions(connectOptions);
        ...
        {
            this.socket.exerciseOptions(tickerId, (Contract)(contract as ComContract), exerciseAction, exerciseQuantity, account, @override, manualOrderTime, customerAccount, professionalCustomer);
        }
        ...
        IOrderCancel ITws.createOrderCancel() { return new ComOrderCancel(); }
        ...
        public delegate void updatePortfolioDelegate(string symbol, string secType, string lastTradeDate, double strike, string right, string currency, string localSymbol, int position, double marketPrice, double marketValue, double averageCost, double unrealizedPNL, double realizedPNL, string accountName);
